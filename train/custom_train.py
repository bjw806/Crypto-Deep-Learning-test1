...
def tester():
    ...
    model.compile(optimizer=tf.keras.optimizers.RMSprop(lr=0.001), loss='categorical_crossentropy', metrics=['acc'])#SGD
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath=target_dir + 'weights-improvement-{epoch:02d}-{acc:.2f}.hdf5',
